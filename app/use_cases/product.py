...
from app.db.models import Category as CatergoryModel
...
class ProductUseCases:
    ...
    def add_product(self, product: Product, category_slug: str):
        category = self.db_session.query(CatergoryModel).filter_by(slug=category_slug).first()
        if not category:
    ...
    def update_product(self,id: int, product: Product):
        ...
        if product_on_db is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='No product was found with this given id')
